...
<div class="col-md-6">
    ...
    <form id="frm" method="post" action="<?php echo $this->module;?>user_edit/<?php echo $data["id"];?>" class="form-horizontal">
        ...
        <div class="control-group">	
			...
			<?php foreach ($group_brwa as $group):?>
            <label class="radio">
            <?php
                $gID=$group['id'];
                $checked = null;
                $item = null;
				// pre($data["group_brwa"]);exit;
				if ($gID == $data["group_brwa"]) {
					$checked= ' checked="checked"';
				}

            ?>
            <input class="group" type="radio" name="groups" data-org="<?php echo $group['use_organization'];?>" value="<?php echo $group['id'];?>"<?php echo $checked;?>>
            <?php echo $group['name'];?>
            </label>
            <?php endforeach?>
