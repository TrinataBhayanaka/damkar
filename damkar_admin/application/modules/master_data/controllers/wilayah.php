...
class wilayah extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->module=$this->folder."master_data/wilayah/";
        $this->http_ref=base_url().$this->module;
        
		$this->load->library(array("form_validation","utils","ion_auth"));

        $this->module_title="Master Data Wilayah";
	...
	function index($forder=0,$limit=10,$page=1){
		...
		// pre($this->juned());
		$data_layout["content"]=$this->load->view("master_data/wilayah/v_wilayah_ajax",$data,true); 
	...
	function dataAjax($forder=0,$limit=10,$page=1,$postKey=false){
		...
		$data_layout["content"]=$this->load->view("master_data/wilayah/v_list",$data,true); 
 	 ...
 	 function addAjax(){
		...
		$data_layout["content"]=$this->load->view("master_data/wilayah/v_add",$this->data,true); 
 	 ...
 	 function edit($idx=false){
		...
		$this->data["process"]=$process;
		// pre($this->data);exit;
		$data_layout["content"]=$this->load->view("master_data/wilayah/v_edit",$this->data,true); 
		if ($data_layout){
            print json_encode(array('status'=>true, 'data'=>$data_layout["content"]));
        }else{
            print json_encode(array('status'=>false));
