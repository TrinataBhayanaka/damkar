...
class tanda_pengenal extends Admin_Controller {
    ...
    function __construct(){
		...
		$class = __CLASS__; //tanda_pengenal
		$this->class=$class;
        ...
        $this->model=new general_model("m_tanda_pengenal");
		...
		$this->module_title="Tanda Pengenal";
		$this->tbl_idx="id";
		$this->tbl_sort="id desc";	
	 }
	 ...
	 function listview(){
        ...
        $totalRows=$this->model->getTotalRecordWhere2($whereSql);
	 ...
	 function add(){
        ...
        if($act=="create"):
			$data=get_post();
			$entitas=$data["name"];
			$entitas1=$data["description"];
			$datam["name"]=$entitas;
			$datam["description"]=$entitas1;
			
			$datas=$this->_add_creator($datam);
			//pre($datas);exit;
			
			$this->conn->StartTrans();
			$this->model->InsertData($datas);
    ...
    function edit($id=null){
		...
		if($act=="update"):
            $data=get_post();
			$entitas1=$data["name"];
			$entitas2=$data["description"];
			$entitas3=$data["created_date"];
			$entitas4=$data["created_by"];
			//$entitas5=$data["id_jenis"];
			
			$datax["name"]=$entitas1;
			$datax["description"]=$entitas2;
			$datax["created_date"]=$entitas3;
			$datax["created_by"]=$entitas4;
			//$datax["id_jenis"]=$entitas5;
	...
	function get_data_uu(){
		...
		if(cek_array($arrDataAll)):
			foreach($arrDataAll as $x=>$val):
				$arrData[$val["id"]]=$val["no_peraturan"]." tentang ".$val["tentang"];
			endforeach;
		endif;
	...
	function update_data_detail($id_parent){
	 ...
	 function lookup_uu(){
        ...
        $offset=$totalRows>$perPage?(int)$this->uri->segment($uriSegment):0;
        $sort=$this->input->get_post("sort")?$this->input->get_post("sort"):"id";
        if(!empty($sort)):
	...
	function delete_peta_file($id=false){
	 ...
	 function delete_file($id=false){
	 	...
	 	$file=$this->conn->GetRow("select * from t_file_upload where id=$id");
	 	$ok=$this->conn->Execute("delete from t_file_upload where id=$id");
		if($ok):
