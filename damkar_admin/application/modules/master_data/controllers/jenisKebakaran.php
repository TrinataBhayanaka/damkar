...
class jenisKebakaran extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->module=$this->folder."master_data/jenisKebakaran/";
	...
	function index($forder=0,$limit=10,$page=1){
		...
		$data_layout["content"]=$this->load->view("master_data/jenisKebakaran/v_jenisKebakaran_ajax",$data,true); 
	...
	function dataAjax($forder=0,$limit=10,$page=1,$postKey=false){
 	 ...
 	 function addAjax(){
		...
		$this->form_validation->set_rules('catKebakaran', "<b>Nama Kebakaran</b>", 'required|xss_clean');
		...
		if ($this->form_validation->run() == true)
		{
			...
			$additional_data = array(
				'catKebakaran'    	=> $this->input->post('catKebakaran'),		
				'status'      		=> 1,
			);
		...
		// exit;
			...
			$dataFormInput=array('catKebakaran');
		...
		$data_layout["content"]=$this->load->view("master_data/jenisKebakaran/v_add",$this->data,true); 
 	 ...
 	 function edit($idx=false){
		...
		$this->data["process"]=$process;
		// pre($this->data);exit;
		$data_layout["content"]=$this->load->view("master_data/jenisKebakaran/v_edit",$this->data,true); 
		if ($data_layout){
            print json_encode(array('status'=>true, 'data'=>$data_layout["content"]));
        }else{
            print json_encode(array('status'=>false));
	...
	function delete($idx=false,$forder=0,$limit=10,$page=1,$postKey=false){
 	 ...
 	 function dataPaging($forder=0,$limit=10,$page=1,$postKey=false){
		...
		$data_layout["content"]=$this->load->view("master_data/jenisKebakaran/v_list",$data,true); 
