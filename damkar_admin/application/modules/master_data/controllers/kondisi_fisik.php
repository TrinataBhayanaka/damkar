...
class kondisi_fisik extends Admin_Controller {
    ...
    function __construct(){
		...
		$class = __CLASS__; //wa_kondisi_fisik
		$this->class=$class;
        ...
        $this->model=new general_model("m_wa_kondisi_fisik");
        $this->load->model("kondisi_fisik_model");
        $this->kondisi_fisik_model=new kondisi_fisik_model();
		$this->main_layout="admin_lte_layout/main_layout";
		$this->module_title="Kondisi Fisik";
		$this->tbl_idx="idx";
	 ...
	 function up($id){
	 	...
	 	$current=0;
	 	$arrData=$this->kondisi_fisik_model->category_search_record_where(false," order by order_num");
	 	...
	 	$this->conn->AutoExecute($this->kondisi_fisik_model->tbl_kondisi_fisik,$data,"UPDATE","idx=".$idBefore);
	 	$data["order_num"]=$current;
	 	$this->conn->AutoExecute($this->kondisi_fisik_model->tbl_kondisi_fisik,$data,"UPDATE","idx=".$idCurrent);
	 ...
	 function down($id){
	 	...
	 	$this->conn->StartTrans();
	 	$data["order_num"]=$current+1;
	 	$this->conn->AutoExecute($this->kondisi_fisik_model->tbl_kondisi_fisik,$data,"UPDATE","idx=".$idNext);
	 ...
	 function add(){
        ...
        if($act=="create"):
			$data=get_post();
			$entitas=$data["kondisi_fisik"];
			$data["order_num"]=$data["order_num"]?$data["order_num"]:$this->conn->GetOne("select max(order_num)+1 from ".$this->kondisi_fisik_model->tbl_kondisi_fisik);
			$entitas1=$data["order_num"];
			$data["kondisi_fisik"]=$entitas;
			$data["order_num"]=$entitas1;
    ...
    function edit($id=null){
		...
		if($act=="update"):
			...
			$entitas=$data["kondisi_fisik"];
			$entitas1=$data["order_num"];
			$datam["kondisi_fisik"]=$entitas;;
			$datam["order_num"]=$entitas1;
			
			//$dataz=$this->_add_editor($datam);
			//pre($dataz);exit;
			
			$this->conn->StartTrans();
			$this->model->UpdateData($datam, "{$this->tbl_idx}=$id");
