...
class sektor extends Admin_Controller {
	...
	function index($forder=0,$limit=10,$page=1){
		...
		$data_layout["content"]=$this->load->view("wilayah/sektor/v_sektor_ajax",$data,true); 
	...
	function dataAjax($forder=0,$limit=10,$page=1,$postKey=false){
 	 ...
 	 function addAjax(){
		...
		// exit;
			...
			$this->data=$this->form_validation->set_dataInput($dataFormInput);
	...
	function delete($idx=false,$forder=0,$limit=10,$page=1,$postKey=false){
 	 ...
 	 function dataPaging($forder=0,$limit=10,$page=1,$postKey=false){
		...
		$data_layout["content"]=$this->load->view("wilayah/sektor/v_list",$data,true); 
