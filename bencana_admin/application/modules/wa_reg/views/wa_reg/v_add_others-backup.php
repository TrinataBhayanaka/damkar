						...
						<div class="col-md-6">
							...
							<div class="row">
								<div class="col-md-6">
									...
									<?php
									$arrPropinsi=m_lookup("propinsi2","kode_bps","nama");
									$idp = explode(',', $arrdata['propinsi']);
									foreach($idp as $vx):
										foreach($arrPropinsi as $k => $vs):
											$hhh[$k]=$vs;
											if(str_replace(' ', '', $vx) == str_replace(' ', '', strtoupper($vs))):
												$hj[] = $k;
											endif;
										endforeach;
									endforeach;
									...
									if(($userSess['group_brwa'] == 1) || ($userSess['group_brwa'] == 2)):
										echo form_dropdown("id_propinsi",$arrPropinsi1,0,"id='id_propinsi' class='form-control required'");
									else:
										echo form_dropdown("id_propinsis",$arrPropinsi1,$idprov,"id='id_propinsi' disabled class='form-control required'");
										echo "<input type='hidden' name='id_propinsi' value='$idprov'>";
									endif;
									?>
									</div>
								</div>
...
$(document).ready(function () {
   ...
   $("#id_propinsi").ready(function(){
   		var idprov = <?=$idprov;?>;
		// alert(idprov);
		var nm_propinsi = $("#id_propinsi option:selected").text();
		$("#id_kabupaten_holder").load("<?=$this->module;?>get_kab_kota/"+idprov+"/?time="+new Date().getTime(),function(){
