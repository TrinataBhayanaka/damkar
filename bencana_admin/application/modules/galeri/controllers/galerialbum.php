...
class galerialbum extends Admin_Controller {
    ...
    function __construct(){
		...
		$this->module=$this->folder."galeri/galerialbum/";
        $this->http_ref=base_url().$this->module;
        
		$this->load->library(array("form_validation","utils","ion_auth"));

        $this->module_title="galerialbum List";
   ...
   function delete($idx=false){
		...
		$data["delete"]=true;
		set_message("success","galerialbum Deleted.");
		// exit;
		redirect("galeri/galerialbum/");
  }
   ...
   function index($forder=0,$limit=10,$page=1){
		...
		$key = ($_GET['q'])?$_GET['q']:0;
		if ($key) {
			$filter = "(category=6) AND (title like '%".$key."%')";
			$data["key"]=$key;
		}
		$offset 		= ($page-1)*$limit;
		$data_start 	= $offset+1;
		...
		// pre($data);exit;
		// exit;
		$data_layout["content"]=$this->load->view("galeri/list_galerialbum",$data,true); 
	...
	function add(){
		...
		$data_layout["content"]=$this->load->view("galeri/add",$data,true); 
		$this->load->view($this->admin_layout,$data_layout);
	}
     ...
     function edit($idx=false){
		...
		$data["acc_active"]="content";
		$data["module"]=$this->module;
		$data_layout["content"]=$this->load->view("galeri/edit",$data,true); 
		$this->load->view($this->admin_layout,$data_layout);
  }
