...
class jenis_ekosistem extends Admin_Controller {
	var $arr_category=array();   
    function __construct(){
        parent::__construct();       
        $this->load->helper(array('form', 'url','file'));
    	$this->load->helper("lookup");
        $class_folder = basename(dirname(__DIR__)); //master_data
        $this->load->library("utils");
        
		$class = __CLASS__; //wa_jenis_ekosistem
		$this->class=$class;
		$this->$class_folder=$class_folder;//master_data
        ...
        $this->model=new general_model("m_wa_jenis_ekosistem");
        $this->load->model("jenis_ekosistem_model");
        $this->jenis_ekosistem_model=new jenis_ekosistem_model();
		$this->main_layout="admin_lte_layout/main_layout";
		$this->module_title="Jenis Ekosistem";
		$this->tbl_idx="idx";
	 ...
	 function up($id){
	 	...
	 	$current=0;
	 	$arrData=$this->jenis_ekosistem_model->category_search_record_where(false," order by order_num");
	 	...
	 	$this->conn->AutoExecute($this->jenis_ekosistem_model->tbl_jenis_ekosistem,$data,"UPDATE","idx=".$idBefore);
	 	$data["order_num"]=$current;
	 	$this->conn->AutoExecute($this->jenis_ekosistem_model->tbl_jenis_ekosistem,$data,"UPDATE","idx=".$idCurrent);
	 ...
	 function down($id){
	 	...
	 	$this->conn->StartTrans();
	 	$data["order_num"]=$current+1;
	 	$this->conn->AutoExecute($this->jenis_ekosistem_model->tbl_jenis_ekosistem,$data,"UPDATE","idx=".$idNext);
	 ...
	 function add(){
        ...
        if($act=="create"):
			$data=get_post();
			$entitas=$data["id_jenis_ekosistem"];
			$entitas1=$data["jenis_ekosistem"];
			$data["order_num"]=$data["order_num"]?$data["order_num"]:$this->conn->GetOne("select max(order_num)+1 from ".$this->jenis_ekosistem_model->tbl_jenis_ekosistem);
			$entitas2=$data["order_num"];
			$data["id_jenis_ekosistem"]=$entitas;
			$data["jenis_ekosistem"]=$entitas1;
			$data["order_num"]=$entitas2;
			
			//$datas=$this->_add_creator($datam);
			//pre($data);exit;
    ...
    function edit($id=null){
		...
		if($act=="update"):
            $data=get_post();
			$entitas1=$data["id_jenis_ekosistem"];
			$entitas2=$data["jenis_ekosistem"];
			$entitas3=$data["order_num"];
			
			$datax["id_jenis_ekosistem"]=$entitas1;
			$datax["jenis_ekosistem"]=$entitas2;
			$datax["order_num"]=$entitas3;
			
			$dataz=$this->_add_editor($datax);
			//pre($dataz);exit;
			
			$this->conn->StartTrans();
			$this->model->UpdateData($dataz, "{$this->tbl_idx}=$id");
			//print_r($this->tbl_idx=$id);exit;
			/*if ($this->model->UpdateData($data, "{$this->tbl_idx}=$id")) {
				echo "sukses";
			}else{
				echo "GAGALA";
			}exit;*/
