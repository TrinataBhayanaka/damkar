...
function compareNoPKO($data){

	$explodeNO_PKO=explode("/",$data);
	$count=count($explodeNO_PKO);
	$last=$explodeNO_PKO[$count-1];
	$beforelast=$explodeNO_PKO[$count-2];
	return array('tahun'=>$last,'bulan'=>$beforelast);

}
function exploDate($data){
	if(!$data){
		$Result=0000-00-00;
	}else{
	$dataExplode=explode("/",$data);
	$Result=$dataExplode[2]."-".$dataExplode[0]."-".$dataExplode[1];
	}
	return $Result;
}

function exploDateStrtotime($data){
...
function exploDateSlash($data){
	
	$dataExplode=explode("-",$data);
	$Result=$dataExplode[1]."/".$dataExplode[2]."/".$dataExplode[0];
