...
<div class="main_content" style="padding:42px 0 10px 0">
	<div class="row-fluid">
    <div class="span12">
        ...
        init: function() {
			...
			if($(window).width() > 979){
                if(!$('body').hasClass('sidebar_hidden')) {
                    if( $.cookie('gebo_sidebar') == "hidden") {
						...
						$('#breadcrumbs.breadcrumbs-fixed').toggleClass('sb_on sb_off');
                    }
                } else {
                    $('.sidebar_switch').toggleClass('on_switch off_switch').attr('title','Show Sidebar');
					$('#breadcrumbs.breadcrumbs-fixed').toggleClass('sb_on sb_off');
                ...
                $('body').addClass('sidebar_hidden');
				$('#breadcrumbs.breadcrumbs-fixed').addClass('sb_off');
            }
            ...
            $('.sidebar_switch').click(function(){
                ...
                if( $('body').hasClass('sidebar_hidden') ) {
					...
					$('#breadcrumbs.breadcrumbs-fixed').addClass('sb_on').removeClass('sb_off');
                } else {
                    $.cookie('gebo_sidebar', 'hidden');
                    $('body').addClass('sidebar_hidden');
                    $('.sidebar_switch').addClass('off_switch');
                    $('.sidebar_switch').attr( 'title', "Show Sidebar" );
					$('#breadcrumbs.breadcrumbs-fixed').addClass('sb_off').removeClass('sb_on');
                }
				//gebo_sidebar.info_box();
