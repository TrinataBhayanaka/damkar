...
<p class="tc">DAFTAR HASIL PENETAPAN BATAS DAERAH ANTAR KABUPATEN/ KOTA</p>
...
<table class="table table-condensed table-bordered">
    ...
    <tbody>
    	<?php if(cek_array($arrData)):
        	...
        	<tr class="ttop" valign="top">
            	<td><?=$x+1?></td>
                <td><?=$val["kabupaten_1"]?> - Prov. <?=$val["propinsi_1"]?> berbatasan dengan:
                	<? if(cek_array($val["data_detail"])):?>
                    	<ul>
                        <? $count=1;?>
                    	<? foreach($val["data_detail"] as $xx=>$detail):?>
                        	<? foreach($detail as $dt):?>
                        	<li  style="list-style:none"><?=$count++?>. <?=$dt["kabupaten_2"]?>  <?=$xx!=$val["id_propinsi_1"]?" - Prov. ".$arrPropinsi[$dt["id_propinsi_2"]]:"";?></li>
                            <? endforeach;?>
						<? endforeach;?>
                        </ul>
					<? endif;?>
                <!--<td>
                	<? if(cek_array($val["data_detail"])):?>
                    	<? foreach($val["data_detail"] as $prop_id=>$detail):?>
							<? if($prop_id==$val["id_propinsi_1"]):?>
								<?=$arrPropinsi[$val["id_propinsi_1"]]?> :<br>
                                <ul>
                                <? foreach($detail as $xx=>$valx):?>
                                    <li style="list-style:none"><?=$xx+1?>. <?=ucwords(strtolower($valx["kabupaten_2"]))?></li>
                                <? endforeach;?>
                                </ul>
                                
                            <? else:?>
                            	<?=$arrPropinsi[$prop_id]?> :<br>
                                <ul>
                                <? foreach($detail as $xx=>$valx):?>
                                    <li style="list-style:none"><?=$xx+1?>. <?=ucwords(strtolower($valx["kabupaten_2"]))?> </li>
                                <? endforeach;?>
                ...
                <td><?=$val["uu_terkait"]?$val["uu_terkait"]:""?></td>
                <td><?=$val["peta_digital"]?$val["peta_digital"]:""?></td>
            </tr>	
		<?php endforeach;
			  endif;
		?>
