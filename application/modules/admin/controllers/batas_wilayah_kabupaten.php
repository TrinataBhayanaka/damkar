...
class batas_wilayah_kabupaten extends Admin_Controller {
    ...
    function __construct(){
        ...
        $this->model=new general_model("tb_batas_kabupaten");
		$this->model_detail=new general_model("tb_batas_kabupaten_detail");
		$this->model_file=new general_model("tb_batas_kabupaten_file");
		$this->model_file_peta=new general_model("tb_batas_kabupaten_file_peta");
		$this->model_uu=new general_model("tb_batas_kabupaten_uu");
		...
		$this->module_title="Batas Wilayah Kabupaten";
		$this->tbl_idx="idx";
	...
	*/
	...
	function listview(){
	 ...
	 function add(){
        ...
        if($act=="create"):
			...
			$dataKab[$id_propinsi_1]=$arrPropinsi1_kab;
			
			foreach($data["id_propinsi_2"] as $prop2):
				$dataKab[$prop2]=$this->get_kab_kota_arr($prop2);		
			endforeach;
			...
			$kabupaten_1=isset($arrPropinsi1_kab[$data["id_kabupaten_1"]])?$arrPropinsi1_kab[$data["id_kabupaten_1"]]:"";
			$data["kabupaten_1"]=$kabupaten_1;
			...
			if(cek_array($data["id_kabupaten_2"])):
				$data_parent["id_parent"]=$id_last;
				$data_parent["id_peraturan"]=$id_peraturan;
				$data_parent["id_propinsi_1"]=$id_propinsi_1;
				$data_parent["id_kabupaten_1"]=$data["id_kabupaten_1"];
				$data_parent["kabupaten_1"]=$kabupaten_1;
				$data_detail=array();	
				foreach($data["id_kabupaten_2"] as $x=>$val):
					$value=$val;
					foreach($value as $valx):
						$data_detail_tmp=array();
						$data_detail_tmp=$data_parent;
						$data_detail_tmp["id_propinsi_2"]=$x;
						$data_detail_tmp["id_kabupaten_2"]=$valx;
						$data_detail_tmp["kabupaten_2"]=$dataKab[$x][$valx];
    ...
    function edit($id){
        ...
        if(empty($act)):
			...
			$data_detail_concat=$this->conn->GetAll("select id_parent,id_propinsi_2,group_concat(id_kabupaten_2) as id_kabupaten_2 from tb_batas_kabupaten_detail where id_parent=$id
group by id_parent,id_propinsi_2");
			if(cek_array($data_detail_concat)):
				foreach($data_detail_concat as $x=>$concat):
					$data_tmp=array();
					$data_tmp=$concat;
					$data_tmp["id_kabupaten_2"]=explode(",",$concat["id_kabupaten_2"]);
					$data_detail_concat[$x]=$data_tmp;
				endforeach;
			endif;
			$data["data_detail_concat"]=$data_detail_concat;
			...
			$dataKab[$arrData["id_propinsi_1"]]=$data["arr_kab1"];
			$arrKabSearch=$this->conn->GetAll("select distinct(id_propinsi_2) as id_propinsi_2 from ".$this->model_detail->tbl." where id_parent=$id and id_propinsi_2!='".$arrData["id_propinsi_1"]."'");
			if(cek_array($arrKabSearch)):
				foreach($arrKabSearch as $x=>$val):
					$dataKab[$val["id_propinsi_2"]]=$this->get_kab_kota_arr($val["id_propinsi_2"]);
				endforeach;
			endif;
			$data["dataKab"]=$dataKab;
    ...
    function del($id){
        ...
        if($act=="delete"):
            $this->conn->StartTrans();
				...
				$this->model_detail->DeleteData("id_parent=$id");
				$this->model_uu->DeleteData("id_parent=$id");
				$this->model_file_peta->DeleteData("id_parent=$id");
	...
	function get_kab_kota2($kd_bps_propinsi="",$arr_id=""){
		$arrData=$this->get_kab_kota_arr($kd_bps_propinsi);
		$data["dataKabupaten"]=$arrData;
		$data["arr_id"]=$arr_id;
		$data["kd_bps_propinsi"]=$kd_bps_propinsi;
	...
	function update_data_detail($id_parent){
		...
		if(cek_array($data["id_kabupaten_2"])):
			$data_parent["id_parent"]=$id_parent;
			$data_parent["id_peraturan"]=$id_peraturan;
			$data_parent["id_propinsi_1"]=$id_propinsi_1;
			$data_parent["id_kabupaten_1"]=$data["id_kabupaten_1"];
			$data_parent["kabupaten_1"]=$kabupaten_1;
