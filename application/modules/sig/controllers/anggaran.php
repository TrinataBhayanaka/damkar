...
class anggaran extends CI_Controller {
	...
	//debug();
		...
		$this->module=$this->folder."sig/anggaran/";
		$this->http_ref=base_url().$this->module;
	
		$this->load->model("anggaran_model");
		$this->model=$this->anggaran_model;
	...
	function get_indikator($not=false) {
		...
		$sql="kode_komponen='0002' and status='1'";
	...
	function map_filter($selected=false,$parent=false){
		...
		$this->load->view("anggaran/map_filter",$data);
	}
  	function map_detail($kdw=false,$thn=false,$indikator=false){
		...
		if ($kd22=="00") {
			...
			$arrPDBWilayah=$this->wilayah_model->SearchRecordWhere($sql_p.$sql_tahun);
			//print_r($arrPDB);
		...
		$data["indikator"]=$ind;
		//print_r($arrDB);
		$this->load->view("anggaran/map_detail",$data);
  	}
  	function map_search($kdw=false,$thn=false,$indikator=false){
		...
		$data["indikator"]=$ind;
		//print_r($arrDB);
		$this->load->view("anggaran/map_search",$data);
  	}
	function map_json($kdw=false,$thn=false,$indikator=false){
		...
		foreach($json['features'] as $k=>$v) {
			...
			$popup_content ="Data Sebaran Anggaran Wilayah<BR><span>Area: ".$arr[$v['properties']['kode_wilayah']]['nama_wilayah']."</span><BR>".$thn;
			...
			$json['features'][$k]['properties']['label0']="Data Sebaran Anggaran Wilayah";
	...
	function map_legend($kdw=false,$thn=false,$indikator=false){
  	...
  	function _bd_nice_number($n) {
		...
		else if($n>1000000) return round(($n/1000000),2).' Jt';
...
// return color array
	function _colorSeries($cs=false,$step=16) {
		...
		$colorseries=($cs)?$cs:"ffff00|33a739|003366";
		$step=($step)?$step:16;
