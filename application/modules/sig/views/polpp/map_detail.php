...
<div style="padding:0 15px 0 15px; margin-top:-10px">
    <div class="row-fluid">
        <div class="span7">
            <div class="row-fluid">
                <div class="span12">
                    <h3>&nbsp;Data Satpol PP & Linmas<small class="pull-right" style="color:grey; line-height:50px"> <i class="icon-tag"></i> Indikator: <strong><?php echo $indikator[$indikator_aktif]['kategori'];?></strong></small></h3>
                </div>
            </div>
            <div class="row-fluid">
                <div class="span12" style="padding-top:0">
					...
					?>
							<div class="box" style=" position:relative;padding:0px 0px 5px 2px; margin-top:-10px; margin-bottom:10px;">
								...
								<div class="box-content" style="background:#f8f8f8;padding-left:50px;margin-bottom:5px">
                               		Alamat Kantor: <?php echo $v['Alamat_kantor'];?><br />
							 	<div style="font-size:large"><?php echo $indikator[$indikator_aktif]['kategori'];?> : <?php echo number_format($v[$indikator_aktif],0,0,".");?></div>
					...
					?>
								<?php echo $iv['kategori'];?>: <?=number_format($v[$iv['colom_nilai']],0,0,".");?><br />
					 <?php
							 }}}
					 ?>
							 			</blockquote>
                                    ...
                                    <ul class="inline" style="width:500px; border-left:4px solid #ddd; padding-left:10px">
                                        <li>&bull; Luas Wilayah: <?=number_format($plist_wilayah[0]['luas_wilayah'],0,0,".");?> Km2</li>
                                        <li>&bull; Jumlah Penduduk: <?=number_format($plist_wilayah[0]['jumlah_penduduk'],0,0,".");?></li>
                                        <li>&bull; Jumlah Kabupaten: <?=number_format($plist_wilayah[0]['jumlah_kabupaten'],0,0,".");?></li>
                                        <li>&bull; Jumlah Kecamatan: <?=number_format($plist_wilayah[0]['jumlah_kecamatan'],0,0,".");?></li>
                                        <li>&bull; Jumlah Desa: <?=number_format($plist_wilayah[0]['jumlah_desa'],0,0,".");?></li>
                             </ul>
                             <?php } ?>
							 	</div>
					...
					?>
                            ...
                            <div class='media-body'>
                                <div class="pull-right" style="color:grey"> <i class="icon-time"></i> Tahun <?php echo $tahun;?></div>
                                <div>
                                    <h4 class="media-heading" style="line-height:24px"><?php echo $v['nama_wilayah'];?></h4>
                               		Alamat Kantor: <?php echo $v['Alamat_kantor'];?><br />
                                    <div style="font-size:large"><?php echo $indikator[$indikator_aktif]['kategori'];?> : <?=number_format($v[$indikator_aktif],0,0,".");?></div>                      
                                </div>
                                <br />
                                    <ul class="inline indikator">
							<?php if (is_array($indikator)) { foreach($indikator as $ik=>$iv) { if ($iv['colom_nilai']!=$indikator_aktif) { ?>
                                        <li>&bull; <?php echo $iv['kategori'];?>: <?=number_format($v[$iv['colom_nilai']],0,0,".");?></li>
                            <?php }}} ?>
                                    </ul>
							<?php if (is_array($list_wilayah[$v['kode_wilayah']])) { ?>
                                    <ul class="inline indikator2">
                                        <li>&bull; Luas Wilayah: <?=number_format($plist_wilayah[0]['luas_wilayah'],0,0,".");?> Km2</li>
                                        <li>&bull; Jumlah Penduduk: <?=number_format($list_wilayah[$v['kode_wilayah']]['jumlah_penduduk'],0,0,".");?></li>
                                        <li>&bull; Jumlah Kabupaten: <?=number_format($list_wilayah[$v['kode_wilayah']]['jumlah_kabupaten'],0,0,".");?></li>
                                        <li>&bull; Jumlah Kecamatan: <?=number_format($list_wilayah[$v['kode_wilayah']]['jumlah_kecamatan'],0,0,".");?></li>
                                        <li>&bull; Jumlah Desa: <?=number_format($list_wilayah[$v['kode_wilayah']]['jumlah_desa'],0,0,".");?></li>
                             </ul>
                             <?php } ?>
...
$(document).ready(function () {
	...
	,	protocol: new OpenLayers.Protocol.HTTP({
			url: 'sig/polpp/map_json/<?php echo $kode_wilayah;?>/<?php echo $tahun;?>/<?php echo $indikator_aktif;?>',
			params:{},
