...
?>
	<h4><?=ucwords($k)?></h4>
    <? foreach($v as $kk=>$vv) { ?>
    <label class="radio" title="<?php echo $vv['nama'];?>">
    <input class="pny" name="sb" type="radio" value="<?php echo $vv['colom_nilai'];?>" <?=(($k=="Pegawai" && $kk==0)?'checked="checked"':'')?> /> <?php echo $vv['nama'];?>
    </label>
    <? } ?>
<?php
}
