...
class news extends Public_Controller {
    ...
    function __construct(){
        ...
        $this->folder="";
		$this->module=$this->folder."news/";
		$this->page_active="news/";
	...
	function news_read($idx=false){
		...
		if (!file_exists($img_dir.$arrDB['image']) || !$arrDB['image']) $arrDB['image'] = false;
	...
	function news_list($forder,$limit,$page){
		...
		if (is_array($arrDB)) {
			...
			foreach($arrDB as $k=>$v) {
				...
				$arrDB[$k]['date_formatted']=$this->utils->dateToString($v['created'],0,2);
