...
class Format {
	...
	public function to_tsv(){
		$output=$this->to_csv("\t");
		return $output;
	}
	
	public function to_csv($sep=",")
	{
		//$sep=!empty($sep)?$sep:",";
		$data = $this->_data;
		...
		$output = implode(''.$sep.'', $headings).PHP_EOL;
		foreach ($data as $row)
		{
			$count=0;
			foreach($row as $x=> $val):
				if($count>0):
					$row[$x]=is_numeric($val)?(float)$val:"\"$val\"";
				else:
					$row[$x]="\"$val\"";
				endif;
				$count++;
			endforeach;
			//$output .= '"'.implode('"'.$sep.'"', $row).'"'.PHP_EOL;
			$output.=join($sep,$row).PHP_EOL;
		}
		return $output;
	}
