...
class Pivot
{
    ...
    {
		$this->current_error_reporting=error_reporting();
		error_reporting(0);
        $tmp = $tmpCount = array();
                ...
                case 2:
                    ...
                    foreach ($this->_columnValues as $item) {
                        if ($item instanceof Pivot_Callback) {
							...
							/* add by whoget 8*/
							$tmpCount[$k0][$k1][$reg[$split]][$reg[$column]][$item->getKey()]=isset($tmpCount[$k0][$k1][$reg[$split]][$reg[$column]][$item->getKey()])?$tmpCount[$k0][$k1][$reg[$split]][$reg[$column]][$item->getKey()]:0;
                            $tmpCount[$k0][$k1][$reg[$split]][$reg[$column]][$item->getKey()] ++;
                        }
						/* add by whoget 8*/
						$tmp[$k0][$k1][$reg[$split]][$reg[$column]][$item]=isset($tmp[$k0][$k1][$reg[$split]][$reg[$column]][$item])?$tmp[$k0][$k1][$reg[$split]][$reg[$column]][$item] :0;
    ...
    private function _buildOutput($tmp, $fetchType, $tmpCount)
    {
                ...
                foreach ($tmp as $p0 => $p0Values) {
                    ...
                    foreach (array_keys($this->_splits) as $split) {
                        ...
                        foreach (array_keys($this->_splits[$split]) as $col) {
							...
							//$colValues = $cols[$col];
							//add by whoget
                            $colValues = isset($cols[$col])?$cols[$col]:array();
                            foreach ($this->_columnValues as $_k) {
                                ...
                                /* add by whoget */
									$this->_lineTotal=isset($this->_lineTotal)?$this->_lineTotal:0;
								/*======================*/
								if ($this->_lineTotal) {
									  /* add by whoget */
									$_lineTotal[$k]=isset($_lineTotal[$k])?$_lineTotal[$k]:0;
                                    $_lineTotal[$k] += $value;
                                }
                                if ($this->_fullTotal) {
									  /* add by whoget */
									  $fullTotal[$split][$col][$k]=isset($fullTotal[$split][$col][$k])?$fullTotal[$split][$col][$k]:0;
                ...
                foreach ($tmp as $p0 => $p0Values) {
                    ...
                    foreach ($p0Values as $p1 => $p1Values) {
                        ...
                        foreach (array_keys($this->_splits) as $split) {
                            ...
                            foreach (array_keys($this->_splits[$split]) as $col) {
								/* add by whoget */
								 $cols[$col]= isset($cols[$col])? $cols[$col]:0;
                                $colValues = $cols[$col];
                                foreach ($this->_columnValues as $_k) {
                                    $k = ($_k instanceof Pivot_Callback || $_k instanceof Pivot_Count) ? $_k->getKey() : $_k;
                                    if ($_k instanceof Pivot_Count) {
                                    ...
                                    if ($this->_pivotTotal) {
										$p0Total[$split][$col][$k]=isset($p0Total[$split][$col][$k])?$p0Total[$split][$col][$k]:0;
		...
		error_reporting($this->current_error_reporting);
        return $return;
    }
