...
$out = '<div class="control-group '.($this->error($name) ? " error" : "").'">';
$out .= '<label class="control-label" for="'.$opts['id'].'">'. $label .'</label>';
...
$out .= "<input name=\"$name\" $id $classes type=\"text\" value=\"$value\" />";
if(!empty($opts['prepend'])) $out .= '</div>';
break;
endswitch;
$out.="</div> <!-- /controls ".$opts['id']." -->";
$out .= '</div><!--/control-group -->';
return $out;
}
